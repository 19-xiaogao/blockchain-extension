 ...
 */
// HD分层钱包的好处 
// 1. 通过助记词来创建私钥,助记词方便记忆，和存储。
// 2. 通过助记词可以创建多个私钥,只是修改HD钱包的派生路径的最后一个数字便可。
/**
...
class HDNodeWallet extends BaseWallet {
    ...
    static createRandom(password, path, wordlist) {
        ...
        // TODO: 为什么要生成16字节长度的随机数呢, blockchain private length 不是 256 位吗？换算成字节 也是32个字节呀。
        const mnemonic = Mnemonic.fromEntropy(randomBytes(16), password, wordlist);
        return HDNodeWallet.#fromSeed(mnemonic.computeSeed(), mnemonic).derivePath(path);
    }
    /**
     *  Create an HD Node from %%mnemonic%%.
     *  通过助记词来创建HD分层钱包, path 可以变成 "m/44'/60'/0'/0/1" 和 "m/44'/60'/0'/0/2" 就可以一次推出
     */
